...
{
	/// <summary>
	/// TMPRuleSetting の Inspector の表示を変更するエディタ拡張
	/// </summary>
	[CustomPropertyDrawer( typeof( TMPRuleSetting ) )]
	internal sealed class TMPRuleSettingDrawer : PropertyDrawer
	{
		//==============================================================================
		// プロパティ
		//==============================================================================
		...
		public override void OnGUI( Rect position, SerializedProperty property, GUIContent label )
		{
			using ( new EditorGUI.PropertyScope( position, label, property ) )
			{
				...
				var isOpenRect          = new Rect( position ) { x = position.x + 16 };
				var isLockRect          = new Rect( position ) { y = isOpenRect.yMax + 6, };
				...
				var comment     = commentProperty.stringValue;
				var name        = nameProperty.stringValue;
				var foldoutName = " " + ( string.IsNullOrWhiteSpace( comment ) ? name : comment );

				property.isExpanded = EditorGUI.Foldout( isOpenRect, property.isExpanded, foldoutName );
